...
- **Domain**: Used as a **soft** gate (boost/penalty only), now computed via embedding similarity to per-specialty `DOMAIN_ANCHORS` (`kb_domain_affinity.get_top_domain()`) rather than hardcoded word lists. No hard domain rejection.
...
- **Auto-bind threshold lowered**: `GROUNDING_AUTO_BIND_THRESHOLD` default is now **0.85** (previously 0.92) — entities auto-link to a local candidate more readily before falling back to the LLM Judge.
...
3. **Step 2 – Super-Pass (streaming default):** `super_pass_cleaning_and_ner_streaming()`; entities with `inventory_category`, `service_category`, `service_type`; default model Llama v3p3 70B; per chunk: batch embed → Batch Intent → dispatch to streaming grounding. For transcripts over `LONG_TRANSCRIPT_THRESHOLD_CHARS` (or when `CHUNK_PARALLEL_ENABLED=true`), `kb_ner_chunk_parallel.IncrementalAsrSuperPass` splits the transcript into overlapping chunks (`compute_adaptive_chunk_params`, `chunk_raw_transcript_with_overlap`), extracts signalment once and injects it into each chunk (`extract_signalment` / `inject_signalment_header`), processes chunks in parallel, then merges (`merge_cleaned_segments`, `deduplicate_entities`, `filter_non_actionable_entities`, `correct_entity_kinds`).
4. **Step 2.3 – Grounding:** `process_single_entity_async()` (streaming) or `run_step_2_3_normalization()` (non-streaming). Dual-sync: `search_local_inventory_topk(..., category_hints=...)` and `search_local_services_topk(..., category_hints=..., service_type=entity_service_type)` in parallel; vector retrieval (batch + on-demand embed); merge; auto-bind (threshold `GROUNDING_AUTO_BIND_THRESHOLD`, default 0.85) or LLM Judge; domain consultation fallback for high-stakes unlinked; `enrich_unlinked_manifest_suggestions()` attaches top RAG candidates to entities that stayed unlinked so the dashboard can still show suggestions.
...
For long recordings, Super-Pass cleaning + NER runs as parallel chunk workers instead of one large call:
...
- `IncrementalAsrSuperPass` – orchestrates per-chunk cleaning + NER with a per-chunk worker timeout.
...
- **Domain soft gate — now embedding-based:** `kb_domain_affinity.py` replaces the old hardcoded per-specialty word lists with `DOMAIN_ANCHORS` (representative terms per specialty) embedded once, plus cosine-similarity scoring at query time (`get_top_domain()`). Still used only as a **soft** gate (boost/penalty); no hard domain rejection. Anchor vectors are cached (`KB_DOMAIN_VECTORS_PATH`).
...
4. For Diagnosis/ReasonForVisit: if best score < 0.95 → note-only; else auto-bind or Judge (only service_id can be set). For other kinds: auto-bind if best local score ≥ `GROUNDING_AUTO_BIND_THRESHOLD` (default **0.85**); else decision flow → LLM Judge.
...
1. **Targeted category search:** Look up Consultation services that match the entity's **domain_key** (e.g. orthopaedic, cardiology) — domain_key itself now comes from the embedding-based `kb_domain_affinity.get_top_domain()`.
...
| **Domain affinity anchors**          | OpenAI (same embedding client)  | 1536       | `kb_domain_affinity.py`; `DOMAIN_ANCHORS` per specialty      | Cached to disk at `KB_DOMAIN_VECTORS_PATH`                              |
...
- **Final ranking:** `final_score = (match_score * SOFT_GATE_LOCAL_BASE_WEIGHT) + domain_boost + category_boost + suggestion_boost` when domain soft gate is on (domain_boost now sourced from `kb_domain_affinity.get_top_domain()`; see `docs/LOCAL_GROUNDING_SEARCH_AND_RETRIEVAL.md`).
- **Default threshold** for "best match" in guard/validation: 0.50 (parameter `threshold` in `search_local_inventory_topk` / `search_local_services_topk`) — distinct from the auto-bind decision threshold `GROUNDING_AUTO_BIND_THRESHOLD` (0.85).
...
| **kb_ner_chunk_parallel.py**               | Chunk-parallel Super-Pass for long transcripts: `IncrementalAsrSuperPass`, adaptive chunk sizing, overlap chunking, signalment extraction/injection, cross-chunk merge/dedupe/kind-correction                                                                                                                                                                                                                                                                                                                                                       |
| **kb_ner_batch_intent.py**                | Batch Intent: search_term + family per entity; Category-Locked Guard                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **kb_ner_parallel.py**                    | Grounding (streaming): process_single_entity_async, process_entity_by_route_async; dual_sync = _local_inventory + _local_services (for non–service-only kinds) or _local_services only for Diagnosis/ReasonForVisit (Pharmacy-Free Zone); **every** output path (batch judge, auto-bind, Judge-selected, high-certainty auto-link local) sets local_stock_id = None for Diagnosis/ReasonForVisit; 0.95 certainty wall (note-only if score < 0.95); auto-bind at GROUNDING_AUTO_BIND_THRESHOLD (0.85); batch prefetch + on-demand embed; domain consultation fallback + dedupe for high-stakes unlinked |
...
| **kb_domain_affinity.py**                 | Embedding-based domain soft-gate: `DOMAIN_ANCHORS` per specialty, cosine-similarity domain detection via `get_top_domain()` (replaces the old hardcoded domain word-lists)                                                                                                                                                                                                                                                                                                                                                                          |
...
| **kb_ner_domain.py**                      | Legacy/simple domain detection helper: detect_domain (superseded in the main path by kb_domain_affinity.py's embedding-based detection)                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `IncrementalAsrSuperPass`, `compute_adaptive_chunk_params`, `chunk_raw_transcript_with_overlap` | kb_ner_chunk_parallel   | Step 2 (long transcripts): chunk-parallel cleaning + NER                                                  |
...
| `get_top_domain`                                                        | kb_domain_affinity               | Embedding-based domain soft-gate: closest specialty anchor via cosine similarity                          |
...
- `auto_bind_threshold` parameter / **`GROUNDING_AUTO_BIND_THRESHOLD`** env override (default **0.85**, lowered from 0.92), `llm_judge_threshold` (0.55)
...
- `KB_DOMAIN_VECTORS_PATH`: cache path for `kb_domain_affinity.py`'s embedded domain anchor vectors
...
| **new_master_documentation.md**                        | **This file.** Current source of truth: full pipeline (incl. chunk-parallel Super-Pass, 0.85 auto-bind threshold, Conclusion section, pipeline observability), Phase 2, Doctor UI, benchmarking, deployment, config                              |
